...
public class AuthRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        //2.查询该用户所具备功能
        List<Module> modules = userService.findUserMenus(user.getId());
        ...
        //4.按照返回值对象要求填充数据
        info.setStringPermissions(moduleSet);//set集合就是用户可以操作的模块，集合中的元素都是string类型的，存的是模块的名称
        //5.返回
        return info;
    }
     ...
     *     使用用户名和密码去数据库查询
     * @param token
     * @return 如果此方法返回null，则subject的login方法就会抛出异常
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        if(user != null){
            //创建返回值对象，此处按照当前方法的返回值要求返回即可
            //使用该对象创建时的构造函数调用自定义密码比较器
