...
public class ContractController extends BaseController {
     ...
     */
    @RequestMapping("/list")
    public String list(@RequestParam(defaultValue = "1") int page,@RequestParam(defaultValue = "5") int size)throws Exception{
        //1.创建example对象
        ...
        switch (user.getDegree()){
            ...
            case 4:{
                //普通员工：只能查看自己的数据
                example.createCriteria().andCreateByEqualTo(user.getId());
                break;
            }
            default:{
                break;
    ...
    public String edit(Contract contract){
        ...
        if(UtilFuns.isEmpty(contract.getId())){
            //保存
            //设置合同的创建者，创建者部门
            contract.setCreateBy(user.getId());//当前认证的用户id
            contract.setCreateDept(user.getDeptId());//当前认证的用户的部门id
